...
export function SiteFooter({ categories }: { categories: Category[] }) {
  ...
  return (
    <footer className="mt-auto border-t border-border bg-ink text-paper">
      <div className="mx-auto flex max-w-6xl flex-col items-center gap-6 px-4 py-8 sm:flex-row sm:items-start sm:justify-around sm:gap-6 sm:text-center">
        <div>
          ...
          <ul className="mt-2 space-y-1.5 text-sm">
            <li>
              <Link href="/shop" className="text-paper/80 hover:text-paper">
                {dict.footer.allFamilies}
              </Link>
            </li>
            <li>
              <Link href="/packs" className="text-paper/80 hover:text-paper">
                {dict.footer.familyPacks}
              </Link>
            </li>
            {departments.map((c) => (
              <li key={c.id}>
        ...
        <div>
          ...
          <ul className="mt-2 space-y-1.5 text-sm">
            ...
            <li>
              <Link href="/brand" className="text-paper/80 hover:text-paper">
                {dict.nav.brand}
              </Link>
            </li>
          </ul>
      ...
      <div className="border-t border-paper/15">
        <div className="mx-auto flex max-w-6xl flex-col gap-2 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2 text-[11px] text-paper/70 [&_img]:!h-5">
            <Logo onDark showWordmark={false} />
            <span>
          ...
          <div className="flex items-center gap-1.5">
            {SOCIALS.map(({ label, href, icon: Icon }) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex size-7 items-center justify-center rounded-full bg-paper text-ink transition-colors hover:bg-primary hover:text-primary-foreground [&_svg]:size-3.5"
              >
                <Icon />
              </a>
            ))}
